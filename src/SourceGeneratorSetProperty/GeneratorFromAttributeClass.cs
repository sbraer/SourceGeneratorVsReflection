...
public sealed class GeneratorFromAttributeClass : IIncrementalGenerator
{
    ...
    private static void CreateSourceCode(SourceProductionContext ctx, ImmutableArray<(string TypeName, Accessibility ClassAccessibility, string Namespaces, MasterType MasterType)> collectedClasses)
    {
        ...
        foreach (var info in collectedClasses)
        {
            ...
            foreach (var subType in info.MasterType.SubTypes)
            {
                ...
                tx.WriteLine($"{SyntaxFacts.GetText(info.ClassAccessibility)} static void SetProperty{subType.Classname}({ns}{subType.Classname} obj, ReadOnlySpan<char> propertyName, string? value)");
                tx.WriteLine('{');
                tx.Indent++;
                tx.WriteLine($"switch (propertyName)");
                tx.WriteLine('{');
                tx.Indent++;
                InsertPropertiesInSwitch(ctx, tx, subType);
                tx.WriteLine("default: throw new Exception();");
                tx.Indent--;
                tx.WriteLine("};");
                tx.Indent--;
                tx.WriteLine('}');
    ...
    private static void InsertPropertiesInSwitch(SourceProductionContext context, IndentedTextWriter tx, SubTypeClass subType)
    {
        ...
        {
            tx.Write($"case \"{property.Name}\" : ");

            var clrType = Helper.GetClrTypeName(property.Type);

            tx.Write((clrType.TypeClr, clrType.IsNullable) switch
            {
                (Helper.TypeClr.DateTime, false) => $"obj.{property.Name} = DateTime.Parse(value!)",
                (Helper.TypeClr.DateTime, true) => $"obj.{property.Name} = DateTime.Parse(value!)",
                (Helper.TypeClr.Number, true) => $"obj.{property.Name} = ConversionHelper.Parsex<{clrType.TypeString}>(value!)",
                (Helper.TypeClr.Number, false) => $"obj.{property.Name} = ConversionHelper.Parsex<{clrType.TypeString}>(value!)",
                (Helper.TypeClr.String, true) => $"obj.{property.Name} = value!.ToString()",
                _ => $"obj.{property.Name} = value!.ToString()"
            });
